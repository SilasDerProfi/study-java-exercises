...
public class QuorumConsensus {
	...
	private boolean checkQuorumConditions() {
		...
		int weight = Arrays.stream(mStations).mapToInt(s -> s.getWeight()).sum();
		if (WRITE_QUORUM + WRITE_QUORUM <= weight) {
			System.err.println("Condition 1 violated");
			return false;
		} else if (READ_QUORUM + WRITE_QUORUM <= weight) {
			System.err.println("Condition 2 violated");
			return false;
		}
		return true;
	}
